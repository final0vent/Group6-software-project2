...
public class Setter {
    public static int[][] coordinate_atoms=new int[6][3];

    public int[][] getAtoms(){
        return coordinate_atoms;
    }
    public void rand_place() {
        ...
        for(int i = 0 ; i < 6 ; i++){
            for(int j = 0 ; j < 3 ; j++)
            {
                coordinate_atoms[i][j] = random.nextInt(5);
            }
        }

        //Print the random coordinate number array
        System.out.println(Arrays.deepToString(coordinate_atoms));
    }
    ...
    public void manu_place(){
        ...
        for(int i = 0 ; i < 6 ; i++){
            ...
            coordinate_atoms[i]=letter_to_coordinate.get(point.toUpperCase());
        }
